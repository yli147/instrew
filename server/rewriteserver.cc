...
public:
    ...
    void Translate(uintptr_t addr) {
        ...
        // Use decode_cfg_no_back_edges: decode the entire CFG but inhibit
        // back-edges (branches to already-decoded addresses). This gives us
        // fast CFG-level translation while safely handling loops - back-edge
        // branches exit the lifted function and go through the dispatch loop
        // for proper register save/restore before re-entering the loop header.
        int fail = ll_func_decode_cfg_no_back_edges(rlfn, addr, accesscb, reinterpret_cast<void*>(iwc));
        if (fail) {
